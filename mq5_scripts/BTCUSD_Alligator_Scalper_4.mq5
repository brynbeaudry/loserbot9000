...
//|                         TODO LIST                                |
//+------------------------------------------------------------------+
// - Change candle based measurements into seconds based measurements where appropriate.
...
// Risk Management
input double RISK_PERCENT = 1.0; // Risk per trade (% of account)
...
input int MIN_SLEEPING_BARS = 3;                   // Minimum bars alligator must sleep before breakout
input int MOUTH_OPENING_WINDOW = 10;               // Bars to wait for mouth to open after entry
input double MAX_LINE_SLOPE = 10;                 // Maximum line slope angle (degrees) for "horizontal" lines
input int SLOPE_CHECK_BARS = 3;                    // Bars to analyze for line slope calculation
input double MAX_LINE_SPREAD_DOLLARS = 50.0;       // Maximum dollar distance between any line pair to consider "closed/sleeping"

// Breakout Slope Validation
input double MIN_BREAKOUT_SLOPE = 60;   // slope angle from jaw (degrees) for valid breakout
input int PRICE_BREAKOUT_SLOPE_CALCULATION_BARS = 3; // Bars to validate price trend consistency (not slope calculation)
input double ATR_PRICE_BREAKOUT_TREND_MULTIPLIER = 1.5; // Multiplier for ATR to determine breakout trend
// Trading Controls
...
int OnInit()
{
   ...
   Print("Trend consistency window: ", PRICE_BREAKOUT_SLOPE_CALCULATION_BARS, " bars (for recent trend validation)");
   Print("ATR distance multiplier: ", ATR_PRICE_BREAKOUT_TREND_MULTIPLIER);
...
void AnalyzeAlligatorState()
{
   ...
   // Start breakout monitoring when alligator ready
   if (sleep_bar_count >= MIN_SLEEPING_BARS && current_position_ticket == 0)
   {
   ...
   else if (monitoring_breakout && (sleep_bar_count < MIN_SLEEPING_BARS || current_position_ticket != 0))
   {
   ...
   if (DEBUG_LEVEL >= 1)
   {
      ...
      if (TimeCurrent() - last_simple_debug > 60) // Every 60 seconds
      {
         ...
         if (is_sleeping && sleep_bar_count >= MIN_SLEEPING_BARS)
         {
            if (monitoring_breakout)
               current_status = "🎯 MONITORING: Sleeping " + IntegerToString(sleep_bar_count) + " bars - watching for breakout";
            else
               current_status = "😴 READY: Sleeping " + IntegerToString(sleep_bar_count) + " bars - will monitor when no position";
         }
         else if (is_sleeping)
         {
   ...
   if (DEBUG_LEVEL >= 2)
   {
      ...
      if (TimeCurrent() - last_debug > 30) // Every 30 seconds
      {
         ...
         else if (sleep_bar_count >= MIN_SLEEPING_BARS && current_position_ticket != 0)
         {
...
void CheckPriceBreakoutFromJaw()
{
   ...
   if (ArraySize(price_history) < PRICE_BREAKOUT_SLOPE_CALCULATION_BARS)
      return;
   ...
   if (price_beyond_jaw && current_direction == breakout_direction)
   {
       ...
       if (elapsed_seconds >= PeriodSeconds()) // Need at least 1 bar worth of time
      {
         ...
         double required_distance = ATR_PRICE_BREAKOUT_TREND_MULTIPLIER * atr_value;

         // Check trend consistency using recent price history
         ...
         if (slope_valid && distance_valid && direction_valid && trend_consistent)
         {
            if (DEBUG_LEVEL >= 1)
            {
               ...
               Print("  Trend consistency: ✓ (recent ", PRICE_BREAKOUT_SLOPE_CALCULATION_BARS, "-bar history confirms direction)");
            }
...
void UpdatePriceHistory()
{
   ...
   if (ArraySize(price_history) != PRICE_BREAKOUT_SLOPE_CALCULATION_BARS)
      ArrayResize(price_history, PRICE_BREAKOUT_SLOPE_CALCULATION_BARS);

   // Set current live price at index 0
   price_history[0] = price;

   // Fill historical positions with actual closing prices of previous bars
   double close_prices[];
   int bars_needed = PRICE_BREAKOUT_SLOPE_CALCULATION_BARS - 1; // Don't include current bar
...
bool ValidatePriceTrendConsistency(TrendDirection direction)
{
   ...
   int bars_to_check = PRICE_BREAKOUT_SLOPE_CALCULATION_BARS;
   ...
   // Count price movements in the recent window only
   for (int i = 0; i < bars_to_check - 1; i++)
   {
      double current_price = price_history[i];     // More recent
...
void ShowInfoPanel()
{
   ...
   {
      breakout_status = "⏳ Need " + IntegerToString(MIN_SLEEPING_BARS - sleep_bar_count) + " more sleep bars";
   }
